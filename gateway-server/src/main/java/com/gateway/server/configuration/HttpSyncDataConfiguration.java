...
import com.gateway.server.constant.HttpConstants;
import com.gateway.server.sync.AppDataSubscriber;
import com.gateway.server.sync.SyncDataService;
import com.gateway.server.sync.http.AccessTokenManager;
import com.gateway.server.sync.http.HttpSyncDataService;
...
@ConditionalOnProperty(prefix = "mobile.gateway.sync.http", name = "url")
public class HttpSyncDataConfiguration {
     ...
     */
    @Bean
    @ConfigurationProperties(prefix = "mobile.gateway.sync.http")
     ...
     * @param httpConfig         the http config
     * @param restTemplate       the rest template
     * @param appDataSubscribers the auth appDataSubscribers
    ...
    public SyncDataService httpSyncDataService(final ObjectProvider<HttpConfig> httpConfig,
                                               ...
                                               final ObjectProvider<List<AppDataSubscriber>> appDataSubscribers,
        ...
        return new HttpSyncDataService(
                ...
                appDataSubscribers.getIfAvailable(Collections::emptyList),
